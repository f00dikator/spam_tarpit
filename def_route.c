...
int do_somethin(u_char *useless, const struct pcap_pkthdr* pkthdr, const u_char* packet)
{
	...
	libnet_t *l;
	int len, ip_hl, srcport, SYN, ACK, myflags, ret, tcp_hl, data = 0, ehlo = 0, mailfrom = 0, rcpt = 0, winsize = 0, arbitrary = 0;
	int undata = 1;
	...
	if (packet[23] == 0x06) 
	{
		...
		if (ACK)
		{
			...
			else
			{
				...
				re = pcre_compile("^([dD][aA][tT][eE]|[sS][uU][bB][jJ][eE][cC][tT]|[tT][oO]|X-[a-zA-Z\-]+|Message-I[dD]|From|User-Agent|[mM][iI][mM][eE]-[a-zA-Z]+|Content-[tT]ype|Received|[a-zA-Z\_]+):.*",0,&error,&erroffset,NULL);
			...
			tcpp = libnet_build_tcp(
				...
				((req == NULL) && (winsize == 0x00)) ? 0 : 1024,		//window size
				0,								// checksum		
				0,								// urgent pointer
